...
// Read current DB config from db.php file
$val_dbHost = 'localhost';
$val_dbPort = '3306';
$val_dbName = 'barangay_pulpogan_bayanihan';
$val_dbUser = 'root';
$val_dbPass = '';
if (preg_match("/\\$dbHost\s*=\s*'([^']*)'/", file_get_contents(__DIR__ . '/../db.php'), $m)) $val_dbHost = $m[1];
if (preg_match("/\\$dbPort\s*=\s*'([^']*)'/", file_get_contents(__DIR__ . '/../db.php'), $m)) $val_dbPort = $m[1];
if (preg_match("/\\$dbName\s*=\s*'([^']*)'/", file_get_contents(__DIR__ . '/../db.php'), $m)) $val_dbName = $m[1];
if (preg_match("/\\$dbUser\s*=\s*'([^']*)'/", file_get_contents(__DIR__ . '/../db.php'), $m)) $val_dbUser = $m[1];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($formType === 'db_config' && $isITOfficer) {
        ...
        if (empty($errors)) {
            $dbFile = file_get_contents(__DIR__ . '/../db.php');
            $dbFile = preg_replace("/\\$dbHost\s*=\s*'[^']*'/", "\$dbHost = '" . addslashes($newDbHost) . "'", $dbFile);
            $dbFile = preg_replace("/\\$dbPort\s*=\s*'[^']*'/", "\$dbPort = '" . addslashes($newDbPort) . "'", $dbFile);
            $dbFile = preg_replace("/\\$dbName\s*=\s*'[^']*'/", "\$dbName = '" . addslashes($newDbName) . "'", $dbFile);
            $dbFile = preg_replace("/\\$dbUser\s*=\s*'[^']*'/", "\$dbUser = '" . addslashes($newDbUser) . "'", $dbFile);
            $dbFile = preg_replace("/\\$dbPass\s*=\s*'[^']*'/", "\$dbPass = '" . addslashes($newDbPass) . "'", $dbFile);

            if (file_put_contents(__DIR__ . '/../db.php', $dbFile) !== false) {
                $_SESSION['flash_msg']  = 'Database configuration saved to db.php. Changes take effect on next page load.';
